...
// ---- hostile library fixtures (temp harness only) ------------------------
LIB_FILES.unshift({
  path: path.join(OUT_DIR, 'Rachmaninoff - Piano Concerto No 2 in C minor Op 18 - II Adagio sostenuto (complete transcription, both cadenzas, unquantised take 14 FINAL final)' + '.mid'),
  name: 'Rachmaninoff - Piano Concerto No 2 in C minor Op 18 - II Adagio sostenuto (complete transcription, both cadenzas, unquantised take 14 FINAL final)',
  dir: OUT_DIR, size: 1_884_213, modified: Date.now() - 90_000,
});
LIB_FILES.unshift({
  path: path.join(EXTRA_DIR, 'a'.repeat(120) + '.mid'),
  name: 'a'.repeat(120), dir: EXTRA_DIR, size: 12_345, modified: Date.now() - 3600_000,
});
LIB_TAGS_EXTRA = true;

const LIB_TAGS = {};
const LIB_USAGE = {};
const KINDS = ['player', 'selfmidi', 'editor', 'forge', 'preview', 'reveal'];
// The real index keys tags/usage by LOWER-CASED path and uses {c,l,e:[{k,t}]}.
LIB_FILES.forEach((f, i) => {
  const key = f.path.toLowerCase();
  const t = TAGSETS[i % TAGSETS.length];
  if (t.length) LIB_TAGS[key] = t.slice();
  if (i % 3 === 0) {
    LIB_USAGE[key] = {
      c: 1 + (i % 9), l: Date.now() - i * 7_200_000,
      e: Array.from({ length: 1 + (i % 5) }, (_, j) => ({ k: KINDS[(i + j) % 6], t: Date.now() - (i + j) * 3_600_000 })),
    };
  }
});
LIB_TAGS[LIB_FILES[0].path.toLowerCase()] = ['needs quantise badly', 'romantic era', 'concerto', 'left hand only', 'practice slow'];
LIB_TAGS[LIB_FILES[1].path.toLowerCase()] = ['aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'];
LIB_USAGE[LIB_FILES[0].path.toLowerCase()] = {
  c: 128_402, l: Date.now() - 60_000,
  e: Array.from({ length: 14 }, (_, i) => ({ k: KINDS[i % 6], t: Date.now() - i * 3_600_000 })),
};
...
const STORAGE = { root: path.parse(OUT_DIR).root, totalBytes: 1_000_204_886_016, freeBytes: 4_803_355_136, usedBytes: 995_401_530_880 };
...
const HANDLERS = {
  ...
  // ---- library -----------------------------------------------------------
  'library:list': () => ({ dirs: [OUT_DIR, EXTRA_DIR], extra: [EXTRA_DIR], files: LIB_FILES, truncated: true }),
  'library:scan': () => ({
    scanId: 1, dirs: [OUT_DIR, EXTRA_DIR], extra: [EXTRA_DIR], files: LIB_FILES, truncated: true,
  ...
  'library:meta': (payload) => {
    ...
    for (const req of list) {
      ...
      meta[p] = { notes: 148_000 + h * 13, duration: 6600 + (h % 300) + 0.482, bpm: 72 + (h % 96) };
    }
    const out = { meta };
...
const clickRow = (i, mods) => `(() => {
  const rows = document.querySelectorAll('#rows .lrow');
  if (!rows[${i}]) return 'no row ${i} (' + rows.length + ' rows)';
  rows[${i}].dispatchEvent(new MouseEvent('click', Object.assign({ bubbles: true }, ${mods})));
  return 'clicked ${i}';
})()`;
...
const PAGES = [
  {
    ...
    states: [
      { name: 'default' },
      { name: 'row-selected', js: clickRow(0, '{}') },
      { name: 'long-row-selected', js: clickRow(1, '{}') },
      { name: 'multi-select', js: `(() => {
          const rows = document.querySelectorAll('#rows .lrow');
          for (const i of [2, 3, 4, 5]) rows[i] && rows[i].dispatchEvent(new MouseEvent('click', { bubbles: true, ctrlKey: true }));
          return 'sel ' + document.querySelectorAll('#rows .lrow[aria-selected=true]').length;
        })()` },
      { name: 'history-tab', js: `(() => { document.getElementById('tab-history').click(); return 'ok'; })()` },
      { name: 'row-menu', js: `(() => {
          document.getElementById('tab-details').click();
          const b = document.querySelector('#rows .lrow [data-action="menu"]');
          if (!b) return 'no menu button';
          b.dispatchEvent(new MouseEvent('click', { bubbles: true }));
          return 'ok';
        })()` },
      { name: 'confirm-dialog', js: `(() => {
          const m = document.querySelector('.menu.is-open');
          if (m) document.body.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
          document.getElementById('a-delete').click();
          return 'ok';
        })()` },
      { name: 'drop-open', js: `(() => {
          const s = document.getElementById('confirm-scrim'); if (s) s.hidden = true;
          const d = document.getElementById('dropcard');
          d.hidden = false; d.classList.add('is-open');
          document.getElementById('body').classList.add('is-over');
          return 'ok';
        })()` },
      { name: 'grid', js: `(() => {
          const d = document.getElementById('dropcard'); d.classList.remove('is-open'); d.hidden = true;
          document.getElementById('body').classList.remove('is-over');
          document.querySelector('#viewmode [data-mode="grid"]').dispatchEvent(new MouseEvent('click', { bubbles: true }));
          return new Promise((r) => setTimeout(() => r('cards ' + document.querySelectorAll('#cards .lib-card').length), 400));
        })()` },
      { name: 'grid-selected', js: `(() => {
          const c = document.querySelectorAll('#cards .lib-card');
          if (!c.length) return 'no cards';
          c[0].dispatchEvent(new MouseEvent('click', { bubbles: true }));
          c[1] && c[1].dispatchEvent(new MouseEvent('click', { bubbles: true, ctrlKey: true }));
          return 'ok';
        })()` },
      { name: 'empty-filtered', js: `(() => {
          document.querySelector('#viewmode [data-mode="list"]').dispatchEvent(new MouseEvent('click', { bubbles: true }));
          const q = document.getElementById('q');
          q.value = 'zzzqqqnothingmatchesthisatall';
          q.dispatchEvent(new Event('input', { bubbles: true }));
          return new Promise((r) => setTimeout(() => r('empty ' + !document.getElementById('empty').hidden), 400));
        })()` },
      { name: 'one-row', js: `(() => {
          const q = document.getElementById('q');
          q.value = 'campanella';
          q.dispatchEvent(new Event('input', { bubbles: true }));
          return new Promise((r) => setTimeout(() => r('rows ' + document.querySelectorAll('#rows .lrow').length), 400));
        })()` },
      { name: 'no-columns', js: `(() => {
          const q = document.getElementById('q'); q.value = ''; q.dispatchEvent(new Event('input', { bubbles: true }));
          const si = document.getElementById('t-side'); if (si.getAttribute('aria-pressed') === 'true') si.click();
          const ii = document.getElementById('t-insp'); if (ii.getAttribute('aria-pressed') === 'true') ii.click();
          return new Promise((r) => setTimeout(() => r('ok'), 300));
        })()` },
      { name: 'insp-only', js: `(() => {
          const ii = document.getElementById('t-insp'); if (ii.getAttribute('aria-pressed') !== 'true') ii.click();
          return new Promise((r) => setTimeout(() => r('ok'), 300));
        })()` },
      { name: 'sections-collapsed', js: `(() => {
          const si = document.getElementById('t-side'); if (si.getAttribute('aria-pressed') !== 'true') si.click();
          for (const h of document.querySelectorAll('#pane-details .insp-sec-head')) h.click();
          return 'ok';
        })()` },
    ],
  },
